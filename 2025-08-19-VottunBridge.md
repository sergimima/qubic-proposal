# Proposal to include QBridge Smart Contract

## Proposal  
Allow QBridge Smart Contract to be deployed on Qubic.
...
## What is QBridge
QBridge is a next-generation, non-custodial cross-chain bridge that links the Qubic blockchain directly with the Ethereum network.  

Taking cues from the seamless user experience of top bridging protocols, QBridge is engineered to let anyone transfer Qubic tokens between Qubic and ETH in seconds **without sacrificing security or decentralization**.  

**Goal:** unlock friction-free interoperability so builders and users can tap the liquidity, dApp ecosystems, and developer tools of both chains from a single entry point.  

---

## QBridge Key Features
Designed for seamless transfers between Qubic assets and Ethereum:

| **Feature** | **Description** |
|-------------|-----------------|
| **Dual Light-Client Verification (DLV)** | Each chain runs a lightweight verifier of the other’s block headers + Merkle proofs, creating a trust-minimized two-way peg with no centralized custodians. |
| **Mint/Burn Wrapped Assets** | When you move Qubic tokens to Ethereum, the originals are locked and an ERC-20 `wQUBIC` is minted 1-for-1; the reverse path burns the wrapper and unlocks the native Qubic asset, guaranteeing constant supply parity. |
| **Gas-Optimized Batch Relays** | Qubic side fees are negligible; on Ethereum the bridge bundles multiple proofs into a single transaction, amortizing gas and cutting user costs dramatically. |
| **Cross-Chain Message Bus** | Beyond tokens, QBridge carries arbitrary payloads (governance votes, oracle data, contract calls) so builders can create true multi-chain dApps that span Qubic’s compute layer and Ethereum’s DeFi stack. |
| **Instant Liquidity Pools** | For users who can’t wait finality, an LP-backed pool provides “liquidity vouchers” so assets arrive instantly, with LPs earning a small fee—similar to LayerZero or Hop fast lanes. |
| **Unified Wallet UX** | One-click bridge inside the Qubic wallet and popular ETH wallets; progress bar shows confirmations on both chains, hiding complexity from end-users. |
| **Audit by Third Party, Open-Source Security** | All contracts and validator code are open-source and undergo third-party audits; every transfer hash and proof is logged on-chain for full transparency. |
| **Low-Fee Model** | Leveraging Qubic’s near-zero fees, the bridge charges only a low fixed QUBIC fee plus the shared ETH gas, redistributing a portion to dev team, shareholders and LP providers. The proposed fee is **1%**. |
...
From https://github.com/qubic/core/pull/497
